...
import com.xucheng.qa.service.ReplyService;
...
public class ReplyController {
	...
	public Result getThumbup(@PathVariable String rediskey){
		...
		return replyService.getThumbup(rediskey);
	}
	/*点赞*/
	@PutMapping("/thumbup/{commentId}/{userid}/{thumbup}")
	public Result thumbup(@PathVariable String commentId, @PathVariable String userid, @PathVariable int thumbup){
		return replyService.thumbup(commentId, userid, thumbup);
	}
	...
	public Result add(@RequestBody Reply reply  ){
		if(httpServletRequest.getAttribute("claims_user")!=null) {
			...
			return new Result(true, StatusCode.OK, "增加成功", reply1);
		}else{
			return new Result(true, StatusCode.ERROR, "增加失败");
		}
	}
